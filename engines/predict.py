    ...
    def evaluate_test(self, data_manager, epoch=None):
        ...
        for iteration in tqdm(range(num_test_iterations)):
            ...
            measures, lab_measures = metrics(
                X_test_batch, y_test_batch, batch_pred_sequence_test, self.configs, data_manager, self.tokenizer)
            # print(measures)
            total_measures.append(measures)
            # if iteration%10==0:
            #     for iteam in self.configs.measuring_metrics:
            #         temp = 0
            #         for measures in total_measures:
            #             temp += measures[iteam]
            #         print(iteam, ":", temp / (iteration+1))

        for iteam in self.configs.measuring_metrics:
            temp = 0
            for measures in total_measures:
                temp += measures[iteam]
            print(iteam,":",temp/len(total_measures))
